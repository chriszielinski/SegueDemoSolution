...
//    }
    ...
    @IBAction func viewNameAction(_ sender: Any) {
        performSegue(withIdentifier: "showTextView", sender: sender)
    }

    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toViewPhoto" {
            if let photoViewController = segue.destination as? PhotoViewController {
                photoViewController.imageToDisplay = previewImageView.image
            }
        } else if segue.identifier == "showTextView" {
            if let textViewController = segue.destination as? TextViewController {
                textViewController.stringToDisplay = nameTextField.text ?? ""
            }
        }
    }
